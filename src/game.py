...
class Game:
    ...
    def run(self):
        while self.running:
            ...
            # --- Game Over avec option Rejouer ---
            if self.misses >= 3:
                self.score_manager.save_score(self.score)
                game_over = self.font.render("GAME OVER", True, (255,0,0))
                self.screen.blit(game_over, (self.width//2 - 100, self.height//2 - 50))

                replay_text = self.font.render("Cliquez pour rejouer", True, (0,255,0))
